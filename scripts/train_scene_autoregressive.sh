...
# Simple wrapper to launch the single-file QVHighlights scene training script.
# Override any variable via environment (e.g., BATCH_SIZE=4 ./train_scene_autoregressive.sh).
...
{
  echo "[$(date +"%F %T")] Launching autoregressive scene training"
...
DATASET=${DATASET}
ROOT=${ROOT}
INTERNVIDEO_ROOT=${INTERNVIDEO_ROOT}
INTERNVIDEO_CONFIG=${INTERNVIDEO_CONFIG}
INTERNVIDEO_CKPT=${INTERNVIDEO_CKPT}
DEVICE=${DEVICE}
EPOCHS=${EPOCHS}
...
INFERENCE_LIMIT=${INFERENCE_LIMIT}
MAX_SAMPLE_INFER=${MAX_SAMPLE_INFER}
...
EOF
  if [[ "${DATASET}" == "msrvtt_untrimmed" ]]; then
    cat <<EOF
MSRVTT_ANNOTATION=${MSRVTT_ANNOTATION}
MSRVTT_FEAT_ROOT=${MSRVTT_FEAT_ROOT}
MSRVTT_TRAIN_SPLIT=${MSRVTT_TRAIN_SPLIT}
MSRVTT_VAL_SPLIT=${MSRVTT_VAL_SPLIT}
MSRVTT_INFER_SPLIT=${MSRVTT_INFER_SPLIT}
EOF
  elif [[ "${DATASET}" == "activitynet" ]]; then
    cat <<EOF
ACTIVITYNET_ROOT=${ACTIVITYNET_ROOT}
ACTIVITYNET_VIDEO_FEATURES=${ACTIVITYNET_VIDEO_FEATURES}
ACTIVITYNET_TEXT_FEATURES=${ACTIVITYNET_TEXT_FEATURES}
ACTIVITYNET_TRAIN_JSON=${ACTIVITYNET_TRAIN_JSON}
ACTIVITYNET_VAL_JSON=${ACTIVITYNET_VAL_JSON}
ACTIVITYNET_INFER_JSON=${ACTIVITYNET_INFER_JSON}
EOF
  else
    cat <<EOF
...
if [[ -n "${VIDEO_CACHE_ROOT}" ]]; then
  COMMON_ARGS+=(--video-cache-root "${VIDEO_CACHE_ROOT}")
fi

if [[ -n "${TEXT_CACHE_ROOT}" ]]; then
  COMMON_ARGS+=(--text-cache-root "${TEXT_CACHE_ROOT}")
fi

# if [[ -n "${MAX_SAMPLE_INFER}" ]]; then
#   COMMON_ARGS+=(--max-sample-infer "${MAX_SAMPLE_INFER}")
# fi
