...
def compute_batch_all_triplet_loss(preds: torch.Tensor, targets: torch.Tensor, margin: float = 0.2, text_sim_threshold: float = 0.85) -> torch.Tensor:
    if preds.numel() == 0 or targets.numel() == 0:
    ...
    # ---------------- [모니터링용 로직 시작] ----------------
    # 학습엔 영향 안 주고(no_grad), 로그만 찍기 위해 가장 센 오답 찾기
    with torch.no_grad():
        # 오답 후보 행렬 복사
        ...
        # (2) 텍스트가 너무 똑같은 경우도 오답에서 제외 (처방 A 적용)
        tgt_sim = positive @ positive.T
        text_mask = tgt_sim > text_sim_threshold
        monitor_sim.masked_fill_(text_mask, -1e9)
        
        # 가장 센 오답(Hard Negative) 추출
        hard_neg_sim, _ = monitor_sim.max(dim=1)
        
        # 유효한 오답이 있는 경우만 평균 계산
        valid_neg = hard_neg_sim > -100
        if valid_neg.sum() > 0:
            h_neg_val = hard_neg_sim[valid_neg].mean().item()
        else:
            h_neg_val = 0.0
            
        p_val = pos_sim.mean().item()
        print(f" Pos: {p_val:.4f} | HardNeg: {h_neg_val:.4f} | Diff: {p_val - h_neg_val:.4f}")
    # ---------------- [모니터링용 로직 끝] ----------------

    # 2. Batch All Loss 계산 (Vectorized)
    # anchor(i)가 positive(i)보다 negative(j)랑 거리가 좁혀지지 않았으면 Loss
    # 식: sim(neg) - sim(pos) + margin > 0 이면 벌점
    # (N, N) - (N, 1) + scalar
    triplet_loss = sim_matrix - pos_sim.unsqueeze(1) + margin
    
    # 3. Masking (Loss 계산에서 제외할 것들)
    # (1) 자기 자신 (대각선) 제외
    mask = torch.eye(sim_matrix.size(0), device=sim_matrix.device).bool()
    
    # (2) [중요] 텍스트가 너무 유사한 "가짜 오답"도 제외 (처방 A)
    # 이것을 안 하면 텍스트 똑같은 것끼리 밀어내느라 로스가 폭발합니다.
    tgt_sim = positive @ positive.T
    invalid_neg_mask = tgt_sim > text_sim_threshold
    mask = mask | invalid_neg_mask # 대각선 + 유사텍스트 모두 True로
    
    # 마스킹 된 곳은 Loss를 0으로 만듦
    triplet_loss.masked_fill_(mask, 0.0)
    
    # (3) 이미 마진보다 멀리 떨어진(잘 맞춘) 것도 0으로 (ReLU)
    triplet_loss = F.relu(triplet_loss)

    # 4. 평균 계산 (0보다 큰 Loss를 가진 Triplet 개수로 나눔)
...
def compute_hard_triplet_loss(preds: torch.Tensor, targets: torch.Tensor, text_sim_threshold: float = 0.85) -> torch.Tensor:
    if preds.numel() == 0 or targets.numel() == 0:
        return preds.new_tensor(0.0)
    ...
    # 1. 씬-텍스트 유사도 행렬 (모델의 예측값)
    sim_matrix = anchor @ positive.T
    
    # [모니터링] 마스킹 전 정답(Positive) 유사도 평균
    pos_sim_val = sim_matrix.diag().mean().item()

    # 2. 텍스트-텍스트 유사도 행렬 (정답끼리의 유사도)
    # 정답 텍스트끼리 너무 비슷하면(0.85 이상), 그건 '오답'으로 쓰지 말자고 판단하기 위함
    tgt_sim_matrix = positive @ positive.T
    
    # 3. 마스크 생성
    # (1) 자기 자신(대각선)은 당연히 오답에서 제외
    # (2) 텍스트 유사도가 임계값(0.85)보다 높은 경우도 오답에서 제외 (True Negative가 아니라고 판단)
    #     예: "앉아있는 사람" vs "의자에 앉은 사람" -> 문장은 다르지만 의미가 같으므로 오답 처리하면 안됨
    invalid_neg_mask = tgt_sim_matrix > text_sim_threshold
    
    # 4. 마스킹 적용 (-inf로 채워서 argmax 선택지에서 탈락시킴)
    sim_matrix.masked_fill_(invalid_neg_mask, float("-inf"))
    
    # 5. Hard Negative Mining (유효한 오답 중에서 가장 헷갈리는 것 선택)
    hard_neg_idx = sim_matrix.argmax(dim=1)
    
    # [모니터링] 선택된 오답의 유사도 확인
    # -inf가 아닌 유효한 값들만 뽑아서 평균 (모두 -inf면 학습 불가 상황)
    neg_sim_values = sim_matrix.gather(1, hard_neg_idx.unsqueeze(1))
    valid_neg_mask = neg_sim_values > -100.0 # -inf 체크
    if valid_neg_mask.sum() > 0:
        neg_sim_val = neg_sim_values[valid_neg_mask].mean().item()
    else:
        neg_sim_val = 0.0 # 유효한 오답이 하나도 없음
    
    # [핵심] 이 Diff가 양수(+)가 되어야 정상입니다.
    print(f" Pos: {pos_sim_val:.4f} | Neg: {neg_sim_val:.4f} | Diff: {pos_sim_val - neg_sim_val:.4f}")

    negatives = positive[hard_neg_idx]
    
    # 6. Loss 계산
    # 오답이 하나도 없어서 negatives가 anchor랑 똑같은 경우(sim_matrix가 다 -inf)를 대비할 필요가 있지만,
    # TripletMarginLoss가 알아서 처리하거나 0이 됨.
    criterion = nn.TripletMarginLoss(margin=0.2, p=2)
    
    # TripletLoss는 기본적으로 거리(Distance) 기반이므로, 유사도(Similarity)와 반대입니다.
    # 코사인 유사도를 쓸 때는 1 - similarity로 거리로 변환해서 넣어주거나,
    # 직접 수식을 쓰는 게 더 직관적일 수 있습니다: max(0, neg_sim - pos_sim + margin)
    # 여기서는 파이토치 함수를 그대로 쓰기 위해 입력값을 유지하되, 
    # 내부적으로는 유클리드 거리를 쓰므로 normalize된 벡터끼리는 코사인 유사도와 밀접한 관계가 있어 작동합니다.
    # 다만, 정확한 '코사인 유사도 기반 마진'을 원하시면 아래 주석처리된 코드를 쓰세요.
    
    # [옵션] 코사인 유사도 전용 수식 (추천)
    # loss = torch.clamp(neg_sim_values - sim_matrix.diag().unsqueeze(1) + 0.2, min=0).mean()
...
def trim_eos_scene_predictions(
    ...
    # 모든 시퀀스에 대해 EOS와의 유사도 계산
    sims = F.cosine_similarity(scene_latents, eos_vector, dim=-1) # (L,)
    
    # Threshold를 넘는 첫 번째 인덱스 찾기
    is_eos = sims >= threshold
    if is_eos.any():
        # 가장 먼저 EOS라고 판명된 위치 찾기
        first_eos_idx = torch.nonzero(is_eos, as_tuple=True)[0][0].item()
        
        # 그 위치 포함(혹은 제외)해서 뒤를 다 날림
        # (EOS 토큰 자체도 필요 없다면 first_eos_idx, 포함하고 싶다면 first_eos_idx + 1)
        # 보통은 EOS 토큰 자체는 보여줄 필요 없으므로 first_eos_idx까지만 슬라이싱
...
class SceneTransformer(nn.Module):
    ...
    class QueryDependentAdapter(nn.Module):
        ...
        def forward(
            ...
            if text_feats.dim() == 2:
                ...
                if text_padding_mask is not None and text_padding_mask.dim() == 1:
                    text_padding_mask = text_padding_mask.unsqueeze(0)
            if text_feats.size(1) == 0:
                return video_feats
            ...
            return self.norm(video_feats + self.dropout(attn_output))
    ...
    def __init__(
        ...
        self.qd_adapter = self.QueryDependentAdapter(embed_dim, num_heads, dropout)
    ...
    def forward(
        ...
        if text_embeddings is not None:
            projected_text = self.project_text_embeddings(text_embeddings)
        else:
            projected_text = None
        query_modulated = self.qd_adapter(
            clip_embeddings,
            projected_text,
            text_padding_mask,
        )
        memory = self._prepare_memory(query_modulated, clip_times, clip_padding_mask)
    ...
    def generate(
        ...
        *,
        ...
        for _ in range(max_steps):
            ...
            stop_prob = torch.sigmoid(self.stop_head(pred_latent)).squeeze(-1)
    ...
    *,
    ...
    with torch.no_grad():
        for batch_samples in data_loader:
            ...
            triplet_loss = compute_batch_all_triplet_loss(valid_preds, valid_targets)
            ...
            total_loss = total_loss + args.lambda_triplet * triplet_loss
...
def collate_scene_batch(
    ...
    *,
    ...
    max_texts = max(t.shape[0] for t in text_embeddings_list)
    text_embed_dim = text_embeddings_list[0].shape[-1]
    text_batch = torch.zeros(batch_size, max_texts, text_embed_dim, device=device)
    text_padding = torch.ones(batch_size, max_texts, dtype=torch.bool, device=device)
    for idx, embeds in enumerate(text_embeddings_list):
        length = embeds.shape[0]
        text_batch[idx, :length, :] = embeds
        text_padding[idx, :length] = False
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch_samples in enumerate(data_loader, start=1):
            ...
            total_loss = total_loss + stop_loss
            stop_loss_scalar = float(stop_loss.item())
            triplet_scalar = float(triplet_loss.item())
            # coverage loss disabled
            ...
            if global_step % args.log_interval == 0:
                logging.info(
                    "Epoch %d step %d | loss=%.4f repr=%.4f attn=%.4f mono=%.4f cov=%.4f triplet=%.4f stop=%.4f",
                    epoch,
                    global_step,
                    total_loss.item(),
                    rep_loss.item(),
