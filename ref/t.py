...
class SceneTransformer(nn.Module):
    ...
    def build_teacher_forcing_inputs(
        ...
        *,
        ...
        inputs = torch.zeros(batch_size, target_len, self.embed_dim, device=device)

        targets = torch.zeros(batch_size, target_len, self.embed_dim, device=device)
        ...
        for idx, latent in enumerate(latents):
            ...
            inputs[idx, 0, :] = self.start_token

            if length > 0:

                inputs[idx, 1 : length + 1, :] = latent[:target_len - 1]

                targets[idx, :length, :] = latent[:target_len]

                scene_mask[idx, :length] = True

            targets[idx, length, :] = self.eos_token

            valid_tokens = min(target_len, length + 1)

            target_padding[idx, :valid_tokens] = False

        return inputs, targets, target_padding, scene_mask, scene_lengths
    ...
    def generate(
        ...
        *,
        ...
        inputs = self.start_token.view(1, 1, -1).to(device)

        padding_mask = torch.zeros(1, 1, dtype=torch.bool, device=device)
        ...
        for _ in range(max_steps):
            ...
            pad_row = torch.zeros(1, 1, dtype=torch.bool, device=device)

            padding_mask = torch.cat([padding_mask, pad_row], dim=1)

            eos_similarity = F.cosine_similarity(

                pred.squeeze(1), self.eos_token.view(1, -1).to(device), dim=-1

            )

            if eos_similarity.item() >= eos_threshold:
